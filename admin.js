document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderAppearanceEditor() {
        ...
        const fontOpts = Object.entries(FONT_OPTIONS).map(([name, cssValue]) =>
            `<option value="${cssValue}" style="font-family: ${cssValue};" ${state.appearance.fontFamily === cssValue ? 'selected' : ''}>${name}</option>`
        ).join('');
